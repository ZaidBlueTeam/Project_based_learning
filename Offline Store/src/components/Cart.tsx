...
export const Cart: React.FC<CartProps> = ({
  ...
  if (cart.length === 0) {
    return (
      <div className={`rounded-lg shadow p-6 border ${theme === 'dark' ? 'bg-gray-800 border-gray-700 text-white' : 'bg-white border-gray-200'}`}>
            ...
            <div key={item.productId} className={`flex items-center gap-4 border-b pb-4 ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'}`}>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className={`w-8 h-8 rounded border hover:bg-gray-100 ${theme === 'dark' ? 'border-gray-600 hover:bg-gray-700' : 'border-gray-300'}`}
                >
                  -
                </button>
